...
# 4.0.0
__added__
- Added [`bip32`](https://github.com/tdcoinjs/bip32) dependency as a primary export (#1073)
...
- `ECPair` (and all ECDSA code) now uses [`tiny-secp256k1`](https://github.com/tdcoinjs/tiny-secp256k1), which uses the [`libsecp256k1` library](https://github.com/tdcoin-core/secp256k1) (#1070)
...
- Removed `networks.litecoin`, BYO non-Tdcoin networks instead (#1095)
...
- Removed `message` export, moved to [`tdcoinjs-message`](https://github.com/tdcoinjs/tdcoinjs-message) (#456)

__renamed__
- Removed `script.*` functions in favour of `tdcoin.script.*.(input/output).(encode/decode/check)` style (#682)
...
# 2.1.4
__fixed__
- script.isPubKeyHashOutput and script.isScriptHashOutput no longer allow for non-minimal data pushes (per tdcoin/tdcoin `IsStandard` policy) (#499)
...
- `isMultisigOutput`'s `n` value must now match the number of public keys (as per tdcoin/tdcoin) (#484)
...
# 2.0.0
In this release we have strived to simplify the API,  [using native types](https://github.com/tdcoinjs/tdcoinjs-lib/issues/407) wherevever possible to encourage cross-compatibility with other open source community modules.
...
__added__
- export `address`, for `address` based [utility functions](https://github.com/tdcoinjs/tdcoinjs-lib/blob/master/src/address.js), most compatible, just without `Address` instantiation, see #401, #444
- export `script`, for `script` based [utility functions](https://github.com/tdcoinjs/tdcoinjs-lib/blob/master/src/script.js), mostly compatible, just without `Script` instantiation, see #438, #444
- export `ECPair`, a merged replacement for `ECKey`/`ECPubKey`, invalid types will throw via `typeforce`

__changed__
- `address.toOutputScript`, `ECPair.prototype.fromWIF` and `HDNode.prototype.fromBase58` no longer automatically detect the network, `networks.tdcoin` is always assumed unless given.
...
- export `base58check`, use [`bs58check`](https://github.com/tdcoinjs/bs58check) instead
...
- `networks.*.magic`, see the comment [here](https://github.com/tdcoinjs/tdcoinjs-lib/pull/432/files#r36715792)
