...
const TESTNET = tdcoin.networks.testnet;

describe('tdcoinjs-lib (addresses)', () => {
  it(
    'can generate a random address [and support the retrieval of ' +
      'transactions for that address (via 3PBP)]',
    async () => {
      const keyPair = tdcoin.ECPair.makeRandom();
      const { address } = tdcoin.payments.p2pkh({ pubkey: keyPair.publicKey });

      // tdcoin P2PKH addresses start with a '1'
  ...
  it('can import an address via WIF', () => {
    const keyPair = tdcoin.ECPair.fromWIF(
  ...
  it('can generate a P2SH, pay-to-multisig (2-of-3) address', () => {
    ...
    const { address } = tdcoin.payments.p2sh({
      redeem: tdcoin.payments.p2ms({ m: 2, pubkeys }),
    });
  ...
  it('can generate a SegWit address', () => {
    ...
    const { address } = tdcoin.payments.p2wpkh({ pubkey: keyPair.publicKey });
  ...
  it('can generate a SegWit address (via P2SH)', () => {
    ...
    const { address } = tdcoin.payments.p2sh({
      redeem: tdcoin.payments.p2wpkh({ pubkey: keyPair.publicKey }),
    });
  ...
  it('can generate a P2WSH (SegWit), pay-to-multisig (3-of-4) address', () => {
    ...
    const { address } = tdcoin.payments.p2wsh({
      redeem: tdcoin.payments.p2ms({ m: 3, pubkeys }),
    });
  ...
  it('can generate a Testnet address', () => {
    const keyPair = tdcoin.ECPair.makeRandom({ network: TESTNET });
    const { address } = tdcoin.payments.p2pkh({
      pubkey: keyPair.publicKey,
      network: TESTNET,
    });

    // tdcoin testnet P2PKH addresses start with a 'm' or 'n'
    assert.strictEqual(
  ...
  it('can generate a Litecoin address', () => {
    // WARNING: although possible, tdcoinjs is NOT necessarily compatible with Litecoin
    const LITECOIN = {
    ...
    const keyPair = tdcoin.ECPair.makeRandom({ network: LITECOIN });
    const { address } = tdcoin.payments.p2pkh({
      pubkey: keyPair.publicKey,
      network: LITECOIN,
    });
