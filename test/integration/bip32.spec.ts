...
function getAddress(node: any, network?: any): string {
  return tdcoin.payments.p2pkh({ pubkey: node.publicKey, network }).address!;
}

describe('tdcoinjs-lib (BIP32)', () => {
  it('can import a BIP32 testnet xpriv and export to WIF', () => {
    ...
    const node = bip32.fromBase58(xpriv, tdcoin.networks.testnet);
  ...
  it('can create a BIP32, tdcoin, account 0, external address', () => {
    const path = "m/0'/0/0";
  ...
  it('can create a BIP44, tdcoin, account 0, external address', () => {
    const root = bip32.fromSeed(
      Buffer.from(
        'dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd',
        'hex',
  ...
  it('can create a BIP49, tdcoin testnet, account 0, external address', () => {
    const mnemonic =
    ...
    const { address } = tdcoin.payments.p2sh({
      redeem: tdcoin.payments.p2wpkh({
        pubkey: child.publicKey,
        network: tdcoin.networks.testnet,
      }),
      network: tdcoin.networks.testnet,
    });
