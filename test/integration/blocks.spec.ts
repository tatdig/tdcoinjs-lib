...
describe('tdcoinjs-lib (blocks)', () => {
  it('can extract a height from a CoinBase transaction', () => {
    ...
    const txHex =
      ...
      '0000000000000';
    const tx = tdcoin.Transaction.fromHex(txHex);

    assert.strictEqual(tx.ins.length, 1);
    const script = tx.ins[0].script;
    // tdcoin.script.decompile(script) // returns [] :(

    assert.strictEqual(script[0], 0x03);
    const heightBuffer = script.slice(1, 4);
    const height = tdcoin.script.number.decode(heightBuffer);
