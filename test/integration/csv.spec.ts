...
function toOutputScript(address: string): Buffer {
  return tdcoin.address.toOutputScript(address, regtest);
}
...
const alice = tdcoin.ECPair.fromWIF(
  'cScfkGjbzzoeewVWmU2hYPUHeVGJRDdFt7WhmrVVGkxpmPP8BHWe',
  regtest,
);
const bob = tdcoin.ECPair.fromWIF(
  'cMkopUXKWsEzAjfa1zApksGRwjVpJRB3831qM9W4gKZsLwjHXA9x',
  regtest,
);
const charles = tdcoin.ECPair.fromWIF(
  'cMkopUXKWsEzAjfa1zApksGRwjVpJRB3831qM9W4gKZsMSb4Ubnf',
  regtest,
);
const dave = tdcoin.ECPair.fromWIF(
  'cMkopUXKWsEzAjfa1zApksGRwjVpJRB3831qM9W4gKZsMwS4pqnx',
  regtest,
);

describe('tdcoinjs-lib (transactions w/ CSV)', () => {
  // force update MTP
  before(async () => {
    await regtestUtils.mine(11);
  });

  const hashType = tdcoin.Transaction.SIGHASH_ALL;
  ...
  function csvCheckSigOutput(
    ...
    return tdcoin.script.fromASM(
      `
      OP_IF
          ${tdcoin.script.number.encode(sequence).toString('hex')}
  ...
  // Ref: https://github.com/tdcoinbook/tdcoinbook/blob/f8b883dcd4e3d1b9adf40fed59b7e898fbd9241f/ch07.asciidoc#complex-script-example

  // Note: tdcoinjs-lib will not offer specific support for problems with
    ...
    return tdcoin.script.fromASM(
      ...
      OP_IF
          OP_IF
              OP_2
          OP_ELSE
              ${tdcoin.script.number.encode(sequence1).toString('hex')}
              OP_CHECKSEQUENCEVERIFY
              OP_DROP
              ${_alice.publicKey.toString('hex')}
              OP_CHECKSIGVERIFY
          ...
          OP_CHECKMULTISIG
      OP_ELSE
          ${tdcoin.script.number.encode(sequence2).toString('hex')}
          OP_CHECKSEQUENCEVERIFY
          OP_DROP
          ${_alice.publicKey.toString('hex')}
          OP_CHECKSIG
  ...
  it(
    ...
    async () => {
      ...
      const p2sh = tdcoin.payments.p2sh({
        redeem: {
      ...
      const tx = new tdcoin.Transaction();
      tx.version = 2;
      ...
      const redeemScriptSig = tdcoin.payments.p2sh({
        network: regtest,
        redeem: {
          network: regtest,
          output: p2sh.redeem!.output,
          input: tdcoin.script.compile([
            tdcoin.script.signature.encode(
              alice.sign(signatureHash),
              hashType,
            ),
            tdcoin.opcodes.OP_TRUE,
          ]),
        },
      }).input;
      ...
      const redeemScriptSig = tdcoin.payments.p2sh({
        ...
        redeem: {
          ...
          input: tdcoin.script.compile([
            ...
            tdcoin.script.signature.encode(bob.sign(signatureHash), hashType),
            tdcoin.opcodes.OP_TRUE,
          ]),
        },
      }).input;
