...
const keyPairs = [
  tdcoin.ECPair.makeRandom({ network: NETWORK }),
  tdcoin.ECPair.makeRandom({ network: NETWORK }),
];
...
async function buildAndSign(
  ...
  const psbt = new tdcoin.Psbt({ network: NETWORK })
    .addInput({
      hash: unspent.txId,
      index: unspent.vout,
...
['p2ms', 'p2pk', 'p2pkh', 'p2wpkh'].forEach(k => {
  ...
  const fn: any = (tdcoin.payments as any)[k];
  ...
  describe('tdcoinjs-lib (payments - ' + k + ')', () => {
    ...
    it(
      'can (as P2WSH(' +
        ...
        ')) broadcast as an output, and be spent as an input',
      async () => {
        const p2wsh = tdcoin.payments.p2wsh({
          redeem: { output },
          network: NETWORK,
        });
