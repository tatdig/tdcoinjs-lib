...
describe('tdcoinjs-lib (transactions with psbt)', () => {
  it('can create a 1-to-1 Transaction', () => {
    const alice = tdcoin.ECPair.fromWIF(
      'L2uPYXe17xSTqbCjZvL2DsyXPCbXspvcu5mHLDYUgzdUbZGSKrSr',
    );
    const psbt = new tdcoin.Psbt();
  ...
  it('can create (and broadcast via 3PBP) a typical Transaction', async () => {
    ...
    const psbt = new tdcoin.Psbt({ network: regtest })
    ...
    // each signer imports
    const signer1 = tdcoin.Psbt.fromBase64(psbtBaseText);
    const signer2 = tdcoin.Psbt.fromBase64(psbtBaseText);
    ...
    const final1 = tdcoin.Psbt.fromBase64(s1text);
    const final2 = tdcoin.Psbt.fromBase64(s2text);
    ...
    // to build and broadcast to the actual Tdcoin network, see https://github.com/tdcoinjs/tdcoinjs-lib/issues/839
  });
  ...
  it('can create (and broadcast via 3PBP) a Transaction with an OP_RETURN output', async () => {
    ...
    const data = Buffer.from('tdcoinjs-lib', 'utf8');
    const embed = tdcoin.payments.embed({ data: [data] });
  ...
  it('can create (and broadcast via 3PBP) a Transaction, w/ a P2SH(P2MS(2 of 4)) (multisig) input', async () => {
    ...
    // build and broadcast to the Tdcoin RegTest network
...
function createPayment(_type: string, myKeys?: any[], network?: any): any {
  ...
  if (isMultisig) {
    ...
    while (!myKeys && n > 1) {
      keys.push(tdcoin.ECPair.makeRandom({ network }));
      n--;
    }
  }
  if (!myKeys) keys.push(tdcoin.ECPair.makeRandom({ network }));

  let payment: any;
  splitType.forEach(type => {
    if (type.slice(0, 4) === 'p2ms') {
      payment = tdcoin.payments.p2ms({
        m,
        pubkeys: keys.map(key => key.publicKey).sort(),
        network,
      });
    } else if (['p2sh', 'p2wsh'].indexOf(type) > -1) {
      payment = (tdcoin.payments as any)[type]({
        redeem: payment,
        network,
      });
    } else {
