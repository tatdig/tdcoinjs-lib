...
describe('Tdcoin-core', () => {
  ...
  describe('address.toBase58Check', () => {
    ...
    base58KeysValid.forEach(f => {
      ...
      const network: any = params.isTestnet
        ? tdcoin.networks.testnet
        : tdcoin.networks.tdcoin;
      const version = network[typeMap[params.addrType]];

      it('can export ' + expected, () => {
        assert.strictEqual(
          tdcoin.address.toBase58Check(hash, version),
          expected,
        );
      });
    });
  ...
  describe('address.fromBase58Check', () => {
    const allowedNetworks = [
      tdcoin.networks.tdcoin.pubKeyHash,
      tdcoin.networks.tdcoin.scriptHash,
      tdcoin.networks.testnet.pubKeyHash,
      tdcoin.networks.testnet.scriptHash,
    ];
    ...
    base58KeysInvalid.forEach(f => {
      ...
      it('throws on ' + strng, () => {
        assert.throws(() => {
          const address = tdcoin.address.fromBase58Check(strng);
  ...
  describe('ECPair', () => {
    base58KeysValid.forEach(f => {
      ...
      const network = params.isTestnet
        ? tdcoin.networks.testnet
        : tdcoin.networks.tdcoin;
      const keyPair = tdcoin.ECPair.fromWIF(strng, network);
  ...
  describe('ECPair.fromWIF', () => {
    ...
    base58KeysInvalid.forEach(f => {
      ...
      it('throws on ' + strng, () => {
        assert.throws(() => {
          tdcoin.ECPair.fromWIF(strng, allowedNetworks);
  ...
  describe('Block.fromHex', () => {
    blocksValid.forEach(f => {
      it('can parse ' + f.id, () => {
        const block = tdcoin.Block.fromHex(f.hex);
  ...
  describe('Transaction.fromHex', () => {
    txValid.forEach(f => {
      ...
      it('can decode ' + fhex, () => {
        const transaction = tdcoin.Transaction.fromHex(fhex as string);
  ...
  describe('Transaction', () => {
    sigHash.forEach(f => {
      ...
      const hashTypes = [];
      if ((hashType & 0x1f) === tdcoin.Transaction.SIGHASH_NONE)
        hashTypes.push('SIGHASH_NONE');
      else if ((hashType & 0x1f) === tdcoin.Transaction.SIGHASH_SINGLE)
        hashTypes.push('SIGHASH_SINGLE');
      else hashTypes.push('SIGHASH_ALL');
      if (hashType & tdcoin.Transaction.SIGHASH_ANYONECANPAY)
      ...
      it(
        ...
        () => {
          const transaction = tdcoin.Transaction.fromHex(txHex);
          assert.strictEqual(transaction.toHex(), txHex);

          const script = Buffer.from(scriptHex, 'hex');
          const scriptChunks = tdcoin.script.decompile(script);
          assert.strictEqual(
            tdcoin.script.compile(scriptChunks!).toString('hex'),
            scriptHex,
          );
  ...
  describe('script.signature.decode', () => {
    sigCanonical.forEach(hex => {
      ...
      it('can parse ' + hex, () => {
        const parsed = tdcoin.script.signature.decode(buffer);
        const actual = tdcoin.script.signature.encode(
          parsed.signature,
          parsed.hashType,
        );
    ...
    sigNoncanonical.forEach((hex, i) => {
      ...
      it('throws on ' + description, () => {
        ...
        assert.throws(() => {
          tdcoin.script.signature.decode(buffer);
        }, reg);
      });
    });
  });
