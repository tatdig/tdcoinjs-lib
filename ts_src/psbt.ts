...
import { tdcoin as TDCNetwork, Network } from './networks';
...
const DEFAULT_OPTS: PsbtOpts = {
  /**
   * A tdcoinjs Network object. This is only used if you pass an `address`
   ...
   * It is only here as a last ditch effort to prevent sending a 500 TDC fee etc.
   */
 ...
 *   Combine checks if the internal tdcoin transaction is the same, so be sure that
...
export class Psbt {
  ...
  setMaximumFeeRate(satoshiPerByte: number): void {
    check32Bit(satoshiPerByte); // 42.9 TDC per byte IS excessive... so throw
 ...
 * It contains a tdcoinjs-lib Transaction object.
 */
